...
def app():
    ...
    # ---- LOAD ASSETS ----
    allEncry_img = Image.open("streamlit/images/allEncry_img.jpg")
    YoloEncry_img = Image.open("streamlit/images/YoloEncry_img.png")
    Yoloseg_img = Image.open("streamlit/images/Yolo_seg_img.png")
    encry_img = Image.open('streamlit/images/encry_img.png')
        ...
        banner_img = """
                        <div id="banner">
                          <div class="banner-content">
                            <h1>智能影像加密系统</h1>
                            <p>基于YOLOv11 + DeepSort的全流程安全防护方案</p>
                          </div>
                        </div>
                """
    ...
    with st.container():
        ...
        with left_column:
            ...
            st.write(
                """
                该系统是针对互联网安全进行的一个图像加密或者解密，主要功能如下
                - 对图片进行自定义加密解密
                - 基于实例分割进行的加密解密 
                """
            )
        ...
        with right_column:
            st.image('streamlit/images/sphere.jpg')
    ...
    with st.container():
        ...
        with st.expander('详解请点击此处'):
            st.write('首先上传一张图片，您可以自己选择加密框的大小与位置，然后根据您的需要，再通过图像加密的方式，只对您认为需要保护的部分进行加密')
    ...
    with st.container():
        text_column, image_column = st.columns((2, 8), gap='medium')
        with image_column:
            st.image(Yoloseg_img)
        with text_column:
            st.subheader("基于实例分割的影像加密")
            st.write(
                """
            ...
            st.write('')
        with st.expander('详解请点击此处'):
            st.write('首先上传一张图片，系统会开始对图片或者视频进行分割，然后根据可以您的需要，选择好分割的部分，再通过图像加密的方式，只对您感兴趣的部分进行加密')
    ...
    with st.container():
        ...
        with text_column:
            ...
            st.write(
                """
                此功能可以对系统加密好的图片进行解密，从而得到与加密前图片一模一样的恢复图片
                """
            )
            st.write(' ')
            st.write('')
        with st.expander('详解请点击此处'):
            st.write('您可以上传需要解密的影像，然后系统将其进行解密')
